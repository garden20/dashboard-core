...
exports.install_app = function(remote_app_details, new_db_name, update_status_function, host_options, callback) {
    ...
    async.waterfall([
        ...
        function(install_doc, callback) {
            install_app_vhosts(host_options, install_doc, update_status_function, callback);
        }
...
function install_app_vhosts (host_options, install_doc, update_status_function, callback) {
...
exports.create_sync_mapping = function(mapping, host_options, callback) {
    ...
    saveSyncMapping(mapping, function(err, sync_doc_id){
        ...
        ensure_sync_doc_dbs(mapping.mapping, function(err){
            ...
            _.each(mapping.mapping, function(sync_doc, index){
                ...
                if (sync_doc.type == 'bi-directional' || sync_doc.type == 'pull') {
                   ...
                   if (sync_doc.install) {
                       ...
                       //TODO we have to do all the stuff that goes with an install!
                       to_record_install.push(sync_doc);
                   }
            ...
            start_replications(rep_docs, function(err){
                ...
                console.log(to_record_install);
                if (to_record_install.length == 0) return callback(null);

                async.forEach(to_record_install, function(sync_doc, cb){
                    record_sync_install(sync_doc, mapping, host_options, cb);
                }, callback);
            });
...
function record_sync_install(sync_doc, main_mapping, host_options, callback){

    console.log(sync_doc);
